exports.up = async function up(knex) {
  const ensureColumn = async (columnName, addColumn) => {
    const exists = await knex.schema.hasColumn('asafo_companies', columnName);
    if (exists) return;
  ...
  await ensureColumn('entry_type', (table) => {
    table.string('entry_type').notNullable().defaultTo('company');
  });
  await ensureColumn('company_key', (table) => {
    table.string('company_key');
  });
  await ensureColumn('title', (table) => {
    table.string('title');
  });
  await ensureColumn('subtitle', (table) => {
    table.string('subtitle');
  });
  await ensureColumn('body', (table) => {
    table.text('body');
  });
  await ensureColumn('display_order', (table) => {
    table.integer('display_order').notNullable().defaultTo(0);
  });
  await ensureColumn('seo_meta_title', (table) => {
    table.string('seo_meta_title');
  });
  await ensureColumn('seo_meta_description', (table) => {
    table.text('seo_meta_description');
  });
  await ensureColumn('seo_share_image', (table) => {
