...
const loadDynamicUrls = async () => {
  const queries = [
    ...
    queryRows("SELECT slug, updated_at FROM about_pages WHERE published = true AND slug IN ('history', 'who-we-are', 'about-agona-nyakrom-town')"),
  ...
  return [
    ...
    ...aboutPages.map((row) => [`/about/${row.slug}`, row.updated_at]),
