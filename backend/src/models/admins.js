...
const create = async (data) => {
  const { email, password_hash, role = 'admin', active = true, last_login_at = null } = data;
  const { rows } = await pool.query(
    `INSERT INTO ${TABLE} (email, password_hash, role, active, last_login_at)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING *`,
    [email, password_hash, role, active, last_login_at]
  );
  return rows[0];
};
