...
const TABLE = 'carousel_items';

const withPagination = async (limit = 10, offset = 0) => {
  const { rows } = await pool.query(
    `SELECT * FROM ${TABLE} ORDER BY sort_order ASC, created_at DESC LIMIT $1 OFFSET $2`,
    [limit, offset]
  );
  return rows;
};
...
const create = async (data) => {
  ...
  const { rows } = await pool.query(
    `INSERT INTO ${TABLE} 
     (title, subtitle, cta_text, cta_url, target_slug, sort_order, active, original_image_path, large_image_path, medium_image_path, thumbnail_image_path)
