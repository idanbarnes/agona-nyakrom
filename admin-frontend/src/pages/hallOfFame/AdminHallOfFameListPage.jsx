...
function AdminHallOfFameListPage() {
  ...
  const fetchHallOfFame = useCallback(async () => {
    ...
    try {
      const payload = await getAllHallOfFame()
  ...
  const handleUnpublish = async (item) => {
    ...
    try {
      const response = await updateHallOfFame(item.id, formData)
      if (response?.success === false) {
        throw new Error(response?.message || 'Unable to unpublish entry.')
      }
      setSuccessMessage('Entry unpublished.')
      fetchHallOfFame()
    } catch (error) {
      const message = error.message || 'Unable to unpublish entry.'
      setError(message)
      window.alert(message)
    }
  }
  ...
  return (
    <section className="mx-auto max-w-6xl space-y-6 pb-8 md:space-y-8">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground md:text-base">
              Manage hall of fame entries, drafts, and publication updates from one place.
            </p>
          </div>
          <Button
            type="button"
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="relative w-full md:max-w-sm">
            ...
            <Input
              ...
              aria-label="Search hall of fame entries"
            />
          </div>
          <div
      ...
      <StateGate
        ...
        empty={
          <EmptyState
            title={hasActiveFilters ? 'No matching entries' : 'No entries found'}
            description={
              hasActiveFilters
                ? 'Try a different name, title, or status filter.'
                : 'Add a Hall of Fame entry to highlight achievements.'
            }
            action={
              hasActiveFilters ? (
                <Button
              ...
              <TableRow key={item.id}>
                <TableCell>
                  {item.photoUrl ? (
                    ...
                    <span className="inline-flex h-10 w-10 items-center justify-center rounded-md border border-dashed border-border bg-muted text-muted-foreground">
                ...
                <TableCell className="text-right whitespace-nowrap">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <Link
                      to={`/admin/hall-of-fame/edit/${item.id}`}
                      className={neutralIconButtonClassName}
