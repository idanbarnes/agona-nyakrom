...
function getLandmarkImageUrl(item) {
  const rawPath =
    item?.images?.medium ||
    item?.images?.large ||
...
function AdminLandmarksListPage() {
  ...
  const normalizedItems = useMemo(() => {
    return items.map((item) => {
      ...
      return {
        ...
        imageUrl: getLandmarkImageUrl(item),
      }
    })
  }, [items])
  ...
  const totalEntries = hasActiveFilters
  ...
  const handleUnpublish = async (item) => {
    ...
    try {
      const response = await updateLandmark(item.id, formData)
      if (response?.success === false) {
        throw new Error(response?.message || 'Unable to unpublish landmark.')
      }
      setSuccessMessage('Landmark unpublished.')
      fetchLandmarks()
    } catch (error) {
      const message = error.message || 'Unable to unpublish landmark.'
      setError(message)
      window.alert(message)
    }
  }
  ...
  return (
    <section className="mx-auto max-w-6xl space-y-6 pb-8 md:space-y-8">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground md:text-base">
              Manage landmarks, drafts, and publication updates from one place.
            </p>
          </div>
          <Button
            type="button"
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="relative w-full md:max-w-sm">
            ...
            <Input
              ...
              placeholder="Search by landmark name or slug"
              className="h-11 w-full pl-10 transition-colors duration-200"
              aria-label="Search landmarks by name or slug"
            />
          </div>
          <div
      ...
      <StateGate
        ...
        empty={
          <EmptyState
            title={hasActiveFilters ? 'No matching landmarks' : 'No landmarks found'}
            description={
              hasActiveFilters
                ? 'Try a different name, slug, or status filter.'
                : 'Add a landmark to highlight places of interest.'
            }
            action={
              hasActiveFilters ? (
                <Button
              ...
              <TableRow key={item.id}>
                ...
                <TableCell className="text-right whitespace-nowrap">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <Link
                      to={`/admin/landmarks/edit/${item.id}`}
                      className={neutralIconButtonClassName}
      ...
      <ConfirmDialog
        ...
        description="Delete this landmark?"
        confirmLabel="Delete"
        variant="danger"
